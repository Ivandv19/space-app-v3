...
function useOrderByLikes(items) { // Define el custom hook que recibe un array de items
    const [topFiveItems, setTopFiveItems] = useState([]); // Inicializa el estado para los cinco items con más likes

    useEffect(() => { // Hook que se ejecuta cuando 'items' cambia
        if (items.length > 0) { // Verifica si hay elementos en el array
